...
class Deeplabv3plus(nn.Module):
	def __init__(self, os):
		...
		# 低特征融合
		self.ll_feats = nn.Sequential(nn.Conv2d(256, 64, 1, bias=False), nn.BatchNorm2d(64)) 
		self.x2_feats = nn.Sequential(nn.Conv2d(512, 256, 1, bias=False), nn.BatchNorm2d(256))
		...
		# 特征图解码器
		self.sal_pred = nn.Sequential(nn.Conv2d(512, 256, 3, stride=1, padding=1, bias=False),
									nn.BatchNorm2d(256), nn.ReLU(),
	...
	def forward(self, input):
		...
		# low_level
		low_level_features = self.ll_feats(x1) # 128，128，265
		
		up_size = low_level_features.size()[2:]
		x2_level_features = self.x2_feats(x2)
		
		x2_level_features = F.interpolate(x2_level_features, up_size, mode='bilinear', align_corners=True)
